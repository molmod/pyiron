...
"""
Parsers for Qchem output files from pymatgen. Adapted for pyiron use.
"""
...
class QCOutput(object):
    ...
    def __init__(self, filename):
        ...
        # In general this will never occur with pyiron
        ...
        # Parse the JOBTYPE
        self.data['jobtype'] = read_pattern(self.text, {'key':r"JOBTYPE\s+(\w+)"}).get('key')[0][0] # list of lists (words after JOBTYPE are captured in groups)

        # Parse the number of alpha and beta electrons and their orbital energies
        self.data['nalpha,nbeta'] = read_pattern(self.text, {'key':r"There are\s+(\d+)\s+alpha and\s+(\d+)\s+beta electrons"}).get('key')[-1]
        self._read_mo_energies()

        self.data['nshells,nbasisfunctions'] = read_pattern(self.text, {'key':r"There are\s+(\d+) shells and\s+(\d+) basis functions"}).get('key')[-1]

        # Load dipole info as well
    ...
    def _read_mo_energies(self):
        """
        Parses the molecular orbital energies
        """
        ...
        orbital_e = read_pattern(self.text,
        {"alpha":r"Alpha MOs, Restricted\s+-- Occupied --(?:\s+(\-?[\d\.]+))",
         "beta" :r"Beta MOs, Restricted\s+-- Occupied --\s+()"})
    ...
    def _read_gradients(self):
        ...
        sorted_gradients = np.zeros(shape=(len(parsed_gradients), len(self.data["last_geometry"]), 3))
        ...
        if self.data["solvent_method"] is not None:
            ...
            pcm_gradients = np.zeros(shape=(len(parsed_gradients), len(self.data["last_geometry"]), 3))
            for ii, grad in enumerate(parsed_gradients):
