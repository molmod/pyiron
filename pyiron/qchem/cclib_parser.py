...
from pyiron.qchem.cclib_data import ccData
...
class Logfile(ABC):
    ...
    def __init__(self, source, logname='Log', **kwds):
    ...
    def parse(self, progress=None, fupdate=0.05, cupdate=0.002):
        ...
        for line in inputfile:
            ...
            try:
                ...
                print("Unexpectedly encountered end of logfile.")
                break
            except Exception as e:
                print("Encountered error when parsing.")
                print("Last line read: %s" % inputfile.last_line)
                raise
    ...
    def set_attribute(self, name, value, check_change=True):
        ...
        if check_change and hasattr(self, name):
            try:
                ...
                #print("Attribute %s changed value (%s -> %s)" % (name, getattr(self, name), value))
                pass
    ...
    def _assign_coreelectrons_to_element(self, element, ncore,
        ...
        """
        atomsymbols = [self.table[atomno].symbol for atomno in self.atomnos]
...
cannot be determined. Rerun without `$molecule read`."""
    ...
    def parse_charge_section(self, inputfile, chargetype):
        ...
        while list(set(line.strip())) != ['-']:
            elements = line.split()
            charge = utils_float(elements[2])
            charges.append(charge)
            if has_spins:
                spin = utils_float(elements[3])
                spins.append(spin)
            line = next(inputfile)
    ...
    def parse_orbital_energies_and_symmetries(self, inputfile):
        ...
        while not self.re_dashes_and_spaces.search(line) \
            ...
            if re.search("[a-zA-Z]", line):
                ...
                for e in tokens:
                    try:
                        energy = utils_float(e) # in hartree
                    except ValueError:
                        energy = numpy.nan
        ...
        if not self.is_fragment_section:
                    ...
                    if line.strip().lower() == '$ecp':
                        ...
                        while line.strip().lower() != '$end':
                            ...
                            while list(set(line.strip())) != ['*']:
                                ...
                                assert element.lower() in table.atoms_by_symbol
                                if len(tokens) > 1:
                                    assert len(tokens) == 2
            ...
            if line.strip() == 'The following effective core potentials will be applied':
                ...
                while list(set(line.strip())) != ['-']:
                    ...
                    if tokens[0] != '':
                        ...
                        ncore = self.table[element].number - valence
            ...
            if line.strip() == "Adding empirical dispersion correction":
                ...
                self.append_attribute(
                    "dispersionenergies",
                    utils_float(line.split()[-2]) # in hartree
                )
            ...
            if 'Standard Nuclear Orientation' in line:
                ...
                while list(set(line.strip())) != ['-']:
                    ...
                    atomcoords.append([convertor(float(value)) for value in entry[2:]]) # in angstrom
                    line = next(inputfile)
                ...
                for atomelement in atomelements:
                    ...
                    if atomelement == 'GH':
                        ...
                        self.atomnos.append(self.table[atomelement].number)
            ...
            if 'SCF converges when ' in line:
                ...
                while not any(message in line for message in scf_success_messages):
                    ...
                    # Is this the end of the file for some reason?
                        ...
                        print('File terminated before end of last SCF! Last error: {}'.format(error))
                        break
            ...
            if 'Total energy in the final basis set' in line:
                ...
                self.scfenergies.append(scfenergy) # in hartree
            ...
            if 'Maximum     Tolerance    Cnvgd?' in line:
                ...
                if not hasattr(self, 'geovalues'):
                    self.geovalues = []
                maxg = utils_float(line_g[0])
                maxd = utils_float(line_d[0])
                ediff = utils_float(line_e[0])
            ...
            if 'MP2         total energy' in line:
                if not hasattr(self, 'mpenergies'):
                    self.mpenergies = []
                mp2energy = float(line.split()[4]) # in hartree
            ...
            if line[1:11] == 'MP2 energy' and line[12:19] != 'read as':
                ...
                self.mpenergies.append(mpenergies) # in hartree
            ...
            if 'CCD total energy' in line:
                if not hasattr(self, 'ccenergies'):
                    self.ccenergies = []
                ccdenergy = float(line.split()[-1]) # in hartree
            ...
            if 'CCSD total energy' in line:
                ...
                if 'CCSD(T) total energy' in line:
                    ...
                    self.ccenergies.append(ccsdtenergy)  # in hartree
                if not has_triples:
                    self.ccenergies.append(ccsdenergy)  # in hartree
            ...
            if 'Excitation Energies' in line:
                ...
                while list(set(line.strip())) != ['-']:
                    ...
                    if 'Total energy for state' in line:
                        ...
                        etenergy = (energy - self.scfenergies[-1])/(planck*lightspeed)/(1/centimeter) # convert to inverse centimeters
            ...
            if "Cartesian Multipole Moments" in line:
                ...
                charge = charge*(centimeter**(3/2) * gram**(1/2) /second)  * 1e-10
            ...
            if 'STANDARD THERMODYNAMIC QUANTITIES AT' in line:
                ...
                if self.natom == 1:
                    ...
                    if not hasattr(self, 'zpe'):
                        ...
                        self.zpe = float(line.split()[4])*kcalmol
                    atommasses = []
                ...
                if not hasattr(self, 'enthalpy'):
                    ...
                    self.enthalpy = enthalpy*kcalmol
                line = next(inputfile)
