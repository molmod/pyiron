...
class PyironUnitRegistry:
    ...
    Usage:
    >>> import pint
    >>> from pyiron.generic.units import PyironUnitRegistry
...
class UnitConverter:
    ...
    def __init__(self, base_registry, code_registry):
        """
        Args:
            base_registry (:class:`pyiron.generic.units.PyironUnitRegistry`): Base unit registry
            code_registry (:class:`pyiron.generic.units.PyironUnitRegistry`): Code specific unit registry
        """
...
class UnitConverter:
    ...
    Module to handle conversions between two different unit registries mainly use to convert units between codes and
    pyiron submodules.
    To instantiate this class, you need two units registries: a base units registry from pint with your desired unit system and a code registry:
    >>> import pint
    >>> base = pint.UnitRegistry(system=system)
    >>> code = PyironUnitRegistry()
    >>> code.add_quantity(quantity="energy",
    ...                         unit=base.kilocal / (base.mol * base.N_A))
    >>> unit_converter = UnitConverter(base_registry=base, code_registry=code)
    The unit converter instance can then be used to obtain conversion factors between code and base units either as a
    `pint` quantity:
    >>> print(unit_converter.code_to_base_pint("energy"))
    ...
    def __init__(self, base_registry, code_registry):
        """
        Args:
            base_registry (:class:`pint.UnitRegistry`): Base unit registry
            code_registry (:class:`pyiron.generic.units.PyironUnitRegistry`): Code specific unit registry
        """
        self._base_registry = base_registry
        self._code_registry = code_registry
