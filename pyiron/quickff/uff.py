...
from molmod.molecular_graphs import MolecularGraph, NRingPattern, HasAtomNumber, HasNeighborNumbers
...
from yaff.pes import InternalCoordinateList
from yaff.pes.dlist import DeltaList
from yaff.pes.iclist import Bond

# This code is largely based on the LJ code from Juul De Vos
...
class UFFMachine():
    ...
    def __init__(self, system, uff_path, max_aromatic_cycle=20, aromatic_rings=[]):
        self.system = system
    ...
    def _init_bond_orders(self):
        ...
        # New implementation, take into account pbc using ICs
        bs = []
        for bond in self.system.iter_bonds():
            bs.append(Bond(*bond))

        bond_vals = np.array(get_ICs(self.system, bs))

        for n,(i0,i1) in enumerate(self.system.iter_bonds()):
            number0 = self.system.numbers[i0]
            number1 = self.system.numbers[i1]
            bond_label = '-'.join(str(i) for i in sorted([i0, i1]))
            bond_order = bonds.bonded(number0, number1, bond_vals[n])
            if bond_order is None:
                print(number0,number1,distance,i0,i1)
    ...
    def read_uff_ffatypes(self, fn):
        if not self.uff_ffatypes == None:
            print('WARNING: Overwritting UFF ffatypes')
    ...
    def _init_uff_ffatypes(self):
        ...
        filters = {'H_': CritAnd(HasAtomNumber(1), is_singular),
                ...
                'O_3_z': CritAnd(HasAtomNumber(8),HasNeighborNumbers(14,14)),
                ...
                'Si3': HasAtomNumber(14), #CritAnd(HasAtomNumber(14), is_tetrahedral),
                #'P_3+3'
                #'P_3+5'
                #'P_3+q'
                #'S_3+2'
        ...
        uff_ffatypes = self.apply_filters(filters, hard=True)
        self.uff_ffatypes = np.array(uff_ffatypes)
    ...
    def construct_bond(self):
        ...
        for i, j in self.system.iter_bonds():
            ...
            try:
                ...
                print(i,j,r_ij)
                raise ValueError('There was a problem with a bond term.')
    ...
    def construct_bend(self):
        ...
        for i, j, k in self.system.iter_angles():
            ...
            # Compute parameters
            geometry = uff_j[2]
            assert geometry in ['1', '2', '3', 'R', '4', '5', '6']
            general = True
            if geometry == '1':
                # Linear
                n = 2
    ...
    def construct_torsion(self):
        ...
        for i, j, k, l in self.system.iter_dihedrals():
            ...
            if uff_l == 'H_':
                ...
                geom_l = uff_l[2]
            assert geom_i in ['1', '2', '3', 'R', '4', '5', '6']
            assert geom_j in ['1', '2', '3', 'R', '4', '5', '6']
            assert geom_k in ['1', '2', '3', 'R', '4', '5', '6']
            assert geom_l in ['1', '2', '3', 'R', '4', '5', '6']
            if geom_i == 'R':
                geom_i = '2'
            if geom_j == 'R':
                geom_j = '2'
            ...
            if geom_j == '3' and geom_k == '3':
                ...
                exception3 = False
                if bond_order == 1.0:
                    group6 = [index for index in pt.iter_numbers() if pt[index].col == 16]
    ...
    def construct_lj(self,ljcross):
        ...
        if ljcross:
            ...
            for i in range(self.system.natom):
                ...
                # Store parameters
                ffatype_key = tuple([self.system.get_ffatype(i)])
                pars_key = (sigma_i, epsilon_i)
                self.add_to(ffatype_lines, prefix, ffatype_key, pars_key)
    ...
    def build(self,ljcross=False):
        ...
        pars_lj = self.construct_lj(ljcross)
        ...
        if ljcross:
            ...
            self.pars_lj = Parameters({'LJ': pars_lj})
        ...
        with open(fn, 'w') as f:
            ...
            for x in globals().values():
                if isinstance(x, type) and issubclass(x, Generator):
                    if x.prefix in ['BONDHARM', 'BENDCOS', 'BENDCHARM', 'TORSION', 'OOPCOS', 'OOPCHARM', 'LJ', 'LJCROSS']:
                        generators[x.prefix] = x()
            for prefix in ['BONDHARM', 'BENDCOS', 'BENDCHARM', 'TORSION', 'OOPCOS', 'OOPCHARM', 'LJ', 'LJCROSS']:
